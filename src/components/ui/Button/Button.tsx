...
const Button = ({ children, reverse, onClick }: { children: string; reverse?: boolean; onClick?: () => void }) => {
	return reverse ? (
		<button
			onClick={onClick}
			type='button'
			className='py-2.5 px-6 text-sm bg-[#f7f7f7] text-[#323332] rounded-full cursor-pointer font-regular text-center shadow-xs transition-all duration-500 hover:bg-stone-200 inline-flex items-center gap-1 capitalize'>
			{children}
